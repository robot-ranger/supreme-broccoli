...
component types in the correct inheritance order.
...
def parse_model(xml_path):
    ...
    Returns a list of dicts: {name, parent, doc, is_abstract, is_leaf, deprecated}
    """
    tree = ET.parse(xml_path)
    root = tree.getroot()

    class_info = _build_class_info(root)
...
def generate_class_code(cls_info):
...
def generate_components_py(xml_components, output_path):
    ...
    # Generate code
    parts = [MODULE_HEADER, DESCRIPTION_TEMPLATE, COMPONENT_TEMPLATE, DEVICE_TEMPLATE]
    ...
    for cls_info in sorted_classes:
        if "organizer" in cls_info:
            ...
            parts.append(generate_class_code(cls_info))
...
def main():
    ...
    components = parse_model(xml_file)
    ...
    generate_components_py(components, output_file)
